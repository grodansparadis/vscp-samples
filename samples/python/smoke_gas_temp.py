#!/usr/bin/python3
"""
// File: send_smoke_temp.py
//
// Usage: send_smoke_temp host user password [guid]
//
...
# get smoke temperature
result = subprocess.run(['./readmax6675'], stdout=subprocess.PIPE)
smoketemp = result.stdout
print(smoketemp)
...
# Login
tn.write(b"user " + user.encode("ascii") + b"\n")
tn.read_until(b"+OK", 2)

tn.write(b"pass " + password.encode("ascii") + b"\n")
...
tempstr = smoketemp.decode('ascii');
...
tn.write(b"send " + event.encode("ascii") + b"\n")
tn.read_until(b"+OK - Success.",2)
